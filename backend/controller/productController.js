...
const validSelection = async ({ brand, size, category }) => {
  if (
    !mongoose.isValidObjectId(brand) ||
    !mongoose.isValidObjectId(size) ||
...
const create = async (req, res) => {
  try {
    if (!(await validSelection(req.body)))
      return res
        .status(400)
        .json({
          success: false,
...
const list = async (req, res) => {
  try {
    ...
    const products = await adminPopulate(
...
const publicList = async (req, res) => {
  try {
    ...
    if (req.query.search?.trim())
      query.name = { $regex: req.query.search.trim(), $options: "i" };
    const products = await publicPopulate(
      Product.find(query).sort({ createdAt: -1 }),
    );
    res.json({ success: true, data: products });
  } catch (error) {
...
const details = async (req, res) => {
  try {
    ...
    const product = await publicPopulate(
      Product.findOne({
...
const adminDetails = async (req, res) => {
  try {
    const product = await adminPopulate(
...
const approve = async (req, res) => {
  try {
    ...
    if (product.approvalStatus === "approved")
      return res
        .status(400)
        .json({ success: false, message: "Product is already approved" });
...
const reject = async (req, res) => {
  try {
    ...
    if (product.approvalStatus === "rejected")
      return res
        .status(400)
        .json({ success: false, message: "Product is already rejected" });
